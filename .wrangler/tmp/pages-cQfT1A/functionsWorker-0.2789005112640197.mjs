...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  if (!printablesUrl?.includes("printables.com/model/")) {
    return new Response(JSON.stringify({ error: "Missing or invalid Printables URL" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
  try {
    let scrapedTitle = "";
    let scrapedImage = "";
    try {
      ...
      if (res.ok) {
        ...
        const getMeta = /* @__PURE__ */ __name((prop) => {
          const match2 = html.match(new RegExp(`<meta[^>]*property="${prop}"[^>]*content="([^"]*)"`, "i"));
          return match2 ? match2[1].replace(/&quot;/g, '"') : "";
        }, "getMeta");
    ...
    const finalPart = {
      title: editedPart?.title || scrapedTitle || "Untitled Part",
      ...
      dropboxZipLastUpdated: editedPart?.dropboxZipLastUpdated || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
      platform: editedPart?.platform || ["Miscellaneous Items"]
    };
    if (!finalPart.title) {
      return new Response(JSON.stringify({ error: "Could not determine title (scraping failed + no input)" }), {
        status: 400,
...
// ../.wrangler/tmp/pages-cQfT1A/functionsRoutes-0.8312560807594276.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-ZS0jne/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ZS0jne/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2789005112640197.mjs.map
